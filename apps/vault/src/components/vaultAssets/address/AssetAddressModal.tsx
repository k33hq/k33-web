...
import { AssetAddress } from '@/types';
import Image from 'next/image';
import CopyIcon from '../../../assets/copy-icon.svg';
import { InfoAlert } from '@/components';

interface AssetAddressModalProps {
...
const AssetAddressModal: React.FC<AssetAddressModalProps> = ({
  assetAddresses,
  visible,
  hide,
  className,
  ...
  const addressRows = assetAddresses.map((vaultAssetAddress, index) => (
    <div
    ...
    >
      ...
      <div className="flex flex-row">
        <div className="grow-0 outline rounded p-1">
          ...
          <Image
            className="inline ps-2 w-8 hover:opacity-70"
            src={CopyIcon}
            alt={'Copy'}
            onClick={() =>
              navigator.clipboard
