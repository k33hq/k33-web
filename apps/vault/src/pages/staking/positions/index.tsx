...
import { Breadcrumbs, VaultStakingPositionsTable } from '@/components';
...
const StakingPositionsPage: NextPageWithLayout = () => {
  ...
  const [query, queryResult] = vaultApi.useLazyGetVaultStakingPositionsQuery();
  useEffect(() => {
    const getVaultStakingPositions = async () => {
      try {
        query();
      } catch (error) {
  ...
  if (queryResult.isLoading) {
    ...
    const vaultStakingPositions = queryResult.data;
    if (!vaultStakingPositions || vaultStakingPositions.length == 0) {
      content = (
        <h1 className="block text-label-light-secondary text-[48px] text-center p-32">
      ...
      content = (
        <VaultStakingPositionsTable
          vaultStakingPositions={vaultStakingPositions}
        />
      );
    }
  }
